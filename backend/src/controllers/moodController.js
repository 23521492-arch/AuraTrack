...
import { calculateMoodStats, getMoodTrend } from "../utils/moodStatistics.js"
...
export const getMoodStats = async (req, res) => {
  try {
    ...
    const { from, to, includeTrend } = req.query
    const q = { userId }
    ...
    const moods = await Mood.find(q).sort({ date: -1 })
    ...
    if (includeTrend === 'true') {
      response.trend = getMoodTrend(moods, 7)
    }
